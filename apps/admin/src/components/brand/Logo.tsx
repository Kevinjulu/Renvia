...
/** An open "A" whose crossbar is cut free of the legs — a floating datum line rather than a typeset bar. */
function StylizedA() {
  return (
...
export function Logo({ className = "", wordmarkClassName = "", showWordmark = true }: LogoProps) {
  return (
    <span className={`inline-flex items-center gap-2 ${className}`}>
      {showWordmark && (
        <span
          className={`inline-flex items-baseline font-display font-medium uppercase tracking-[0.25em] text-primary ${wordmarkClassName || "text-lg"}`}
        >
