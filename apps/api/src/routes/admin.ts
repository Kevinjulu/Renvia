  ...
  const [[users], statusRows, [today], modelRows, [credits], dailyRows, topRows] = await Promise.all([
    ...
    db
      .select({
        ...
        spent: sql<number>`coalesce(-sum(${schema.creditLedger.amount}) filter (where ${schema.creditLedger.reason} in ('render', 'render_refund')), 0)::int`,
      })
...
admin.post("/users/:id/credits", async (c) => {
  ...
  try {
    ...
    const [updated] = await db.batch([
      ...
      db.insert(schema.creditLedger).values({ userId: id, amount, reason: "admin_grant", note, actorId: c.get("admin").id }),
    ]);
...
admin.patch("/users/:id", async (c) => {
  ...
  if (id === c.get("admin").id && (body.disabled === true || body.role === "user")) {
