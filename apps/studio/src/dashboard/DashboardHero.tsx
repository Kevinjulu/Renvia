...
export function DashboardHero({ onCreate }: { onCreate: () => void }) {
  ...
  return <section className="dashboard-hero">
    <div className="dashboard-hero-copy"><p>FROM SKETCH TO REALITY</p><h2>Visualize a better tomorrow</h2><span>Upload your sketch, model, or photo and let Renvia transform it into photorealistic renders — preserving your design, just enhancing the possibilities.</span><div><button type="button" className="dashboard-primary-button" onClick={onCreate}><b>＋</b> Create new project</button><button type="button" className="dashboard-secondary-button" onClick={() => navigate("/help/getting-started")}><b>▷</b> Watch how it works</button></div></div>
