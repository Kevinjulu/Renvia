...
export const users = pgTable("users", {
    ...
    check("users_daily_render_limit_override_range", sql `${table.dailyRenderLimitOverride} IS NULL OR ${table.dailyRenderLimitOverride} BETWEEN 0 AND 10000`),
    check("users_daily_segment_limit_override_range", sql `${table.dailySegmentLimitOverride} IS NULL OR ${table.dailySegmentLimitOverride} BETWEEN 0 AND 10000`),
    check("users_monthly_render_limit_override_range", sql `${table.monthlyRenderLimitOverride} IS NULL OR ${table.monthlyRenderLimitOverride} BETWEEN 0 AND 100000`),
    check("users_monthly_segment_limit_override_range", sql `${table.monthlySegmentLimitOverride} IS NULL OR ${table.monthlySegmentLimitOverride} BETWEEN 0 AND 100000`),
]);
...
export const renders = pgTable("renders", {
    ...
    check("renders_aspect_ratio_values", sql `${table.aspectRatio} IN ('auto', '1:1', '16:9', '4:3', '3:4', '9:16')`),
]);
...
export const appSettings = pgTable("app_settings", {
    ...
    check("app_settings_monthly_limit_range", sql `${table.monthlyRenderLimit} IS NULL OR ${table.monthlyRenderLimit} BETWEEN 1 AND 100000`),
    check("app_settings_monthly_segment_limit_range", sql `${table.monthlySegmentLimit} IS NULL OR ${table.monthlySegmentLimit} BETWEEN 1 AND 100000`),
    check("app_settings_max_credit_balance_range", sql `${table.maxCreditBalance} IS NULL OR ${table.maxCreditBalance} BETWEEN 1 AND 1000000`),
    check("app_settings_max_projects_range", sql `${table.maxProjectsPerUser} IS NULL OR ${table.maxProjectsPerUser} BETWEEN 1 AND 10000`),
    ...
    check("app_settings_max_selection_prompt_chars_range", sql `${table.maxSelectionPromptChars} BETWEEN 10 AND 1000`),
